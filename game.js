...
for(const set of LOOT_SET_SPECS){
  for(const c of CLASS_GEAR){
    ITEMS[`${set.prefix}_${c.key}_weapon`]={name:`${set.name} ${c.weapon}`,type:"weapon",className:c.className,icon:c.weaponIcon,tier:set.tier,attack:set.attack,element:set.weaponEffect,requiredLevel:set.level,value:set.value,salvage:set.salvage,special:true,raid:!!set.raid,source:set.name};
    ITEMS[`${set.prefix}_${c.key}_armor`]={name:`${set.name} ${c.armor}`,type:"armor",className:c.className,icon:c.armorIcon,tier:set.tier,defense:set.defense,element:set.armorEffect,requiredLevel:set.level,value:set.value,salvage:set.salvage,special:true,raid:!!set.raid,source:set.name};
  }
}
...
function startRun(combatId,heroIds,autoRepeat=true){
  ...
  payRun(cfg,heroIds);for(const id of heroIds)heroById(id).assignment="combat";
