...
async function init(){
  ...
  setInterval(()=>{const now=Date.now(),delta=Math.min(5,(now-lastFrame)/1000);lastFrame=now;simulate(delta);if(currentView==="combat")renderCombatLive();if(now-lastSlowRender>=1000){lastSlowRender=now;renderResources();renderTown();if(currentView==="warehouse")renderWarehouse();refreshWorkTimers();markDirty();}},100);
}
