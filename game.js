...
const WORK_SKILL_NAMES = {farming:"Farming",cooking:"Cooking",mining:"Mining",woodcutting:"Woodcutting",smithing:"Smithing"};
...
const WORK_ASSIGNMENTS=["farm","cook","mine","forest","smith"];
...
function workTasksFor(assignment){return RESOURCE_ASSIGNMENTS[assignment]?RESOURCE_TIERS[RESOURCE_ASSIGNMENTS[assignment]]:assignment==="cook"?COOK_WORK_TASKS:assignment==="smith"?SMITH_WORK_TASKS:[];}
function unlockedWorkTasks(h,assignment){if(RESOURCE_ASSIGNMENTS[assignment])return unlockedResourceTiers(h,assignment);if(!["cook","smith"].includes(assignment))return [];const skill=BUILDINGS[assignment].skill,buildingLevel=state.buildings[assignment]||1,tasks=assignment==="cook"?COOK_WORK_TASKS:SMITH_WORK_TASKS;return tasks.filter(task=>(h.skills[skill]?.level||1)>=task.level&&buildingLevel>=task.building);}
...
function freshState(){
  return {
    ...
    heroes:HEROES.map((h,i)=>({ ...h, level:1,xp:0,sanity:100,hp:Math.round(100*(CLASS_COMBAT[h.className]?.hp||1)),assignment:"idle", recoveryUntil:0,workProgress:0,workTiers:{farm:"starter",cook:"starter",mine:"starter",forest:"starter"},records:{...HERO_RECORD_DEFAULTS},
      skills:{farming:{level:1,xp:0},cooking:{level:1,xp:0},mining:{level:1,xp:0},woodcutting:{level:1,xp:0},smithing:{level:1,xp:0}},
...
function renderTown(){
  ...
  const counts=["farm","cook","mine","forest","smith"].map(a=>[a,state.heroes.filter(h=>h.assignment===a).length]);$("#townOutputMini").innerHTML=counts.filter(x=>x[1]).map(([a,n])=>`<div class="mini-bar"><span>${ASSIGNMENTS[a].icon} ${ASSIGNMENTS[a].name}</span><i style="--w:${n/6*100}%"></i><b>${n}</b></div>`).join("")||`<span class="empty-mini">Assign heroes to begin production.</span>`;
...
function renderAssignments(){
  const options=["idle","farm","cook","mine","forest","smith","tavern"];
