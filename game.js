...
function upgradeBuilding(id){const b=BUILDINGS[id],cost=buildingUpgradeCost(id);if(state.resources.gold<cost.gold)return toast("🪙","Not enough Gold");if(state.resources.wood<cost.wood)return toast("🌲","Not enough Wood");if(state.resources.metal<cost.metal)return toast("⛏️","Not enough Metal");state.resources.gold-=cost.gold;spendQuestResource("wood",cost.wood);spendQuestResource("metal",cost.metal);state.buildings[id]++;notify(`${b.name} upgraded`,`Building Level ${state.buildings[id]} is now complete.`,b.icon);const speaker=state.heroes.find(hero=>hero.assignment===id)||chatPick(state.heroes.filter(hero=>hero.assignment==="idle"));if(speaker)postHeroChat(speaker,`${b.name} Level ${state.buildings[id]} is complete. ${RESOURCE_ASSIGNMENTS[id]?"Check our exact tasks—higher materials may be available.":"The town feels a little stronger already."}`,"upgrade");markDirty();renderAll();if(id==="market")openView("market");else openBuilding(id);}
