...
const COMBAT_STANCES={
  balanced:{name:"Balanced",icon:"⚖️",detail:"No modifiers. Reliable in every encounter.",attack:1,defense:1,maxHit:1,damageTaken:1,food:1},
  aggressive:{name:"Aggressive",icon:"⚔️",detail:"+15% Attack and max hit, but -10% Defence.",attack:1.15,defense:.90,maxHit:1.15,damageTaken:1,food:1},
  guarded:{name:"Guarded",icon:"🛡️",detail:"+20% Defence, but -12% max hit.",attack:1,defense:1.20,maxHit:.88,damageTaken:1,food:1},
  cautious:{name:"Cautious",icon:"🥕",detail:"+5% Defence and meals heal 25% more.",attack:1,defense:1.05,maxHit:1,damageTaken:1,food:1.25},
  reckless:{name:"Reckless",icon:"🔥",detail:"+22% max hit, but takes 15% more damage.",attack:1,defense:1,maxHit:1.22,damageTaken:1.15,food:1},
};
const BOND_MILESTONES=[
  {level:25,name:"Familiar Rhythm",detail:"+3% damage while adventuring together."},
  {level:50,name:"Cover Each Other",detail:"Also take 3% less damage while together."},
  {level:75,name:"Perfect Timing",detail:"Also gain +3% Crit while together."},
];
function bondKey(a,b){return [a,b].sort().join(":");}
function bondValue(a,b){return clamp(Number(state?.bonds?.[bondKey(a,b)])||0,0,100);}
function partyBondBonus(run,h){
  const companions=(run?.heroIds||[]).filter(id=>id!==h.id);
  const bonds=companions.map(id=>bondValue(h.id,id));
  return {
    damage:1+bonds.filter(v=>v>=25).length*.03,
    taken:Math.max(.82,1-bonds.filter(v=>v>=50).length*.03),
    crit:bonds.filter(v=>v>=75).length*.03
  };
}
function growPartyBonds(party,category,offline=false){
  const gain=category==="raid"?4:category==="dungeon"?2:1;if(party.length<2)return;
  state.bonds=state.bonds||{};
  for(let i=0;i<party.length;i++)for(let j=i+1;j<party.length;j++){
    const key=bondKey(party[i].id,party[j].id),before=bondValue(party[i].id,party[j].id),after=clamp(before+gain,0,100);state.bonds[key]=after;
    for(const milestone of BOND_MILESTONES)if(before<milestone.level&&after>=milestone.level&&!offline){
      postHeroChat(party[i],`${party[j].name} and I are starting to move like we've done this before.`,"milestone",{mapBubble:false});
      postHeroChat(party[j],`Don't make it weird. But... yeah. Good run.`,"reply",{mapBubble:false});
      notify("Bond deepened",`${party[i].name} + ${party[j].name}: ${milestone.name} (${milestone.level}/100).`,"🤝");
    }
  }
}
function heroBondsHTML(h){
  const rows=state.heroes.filter(x=>x.id!==h.id).map(other=>({other,value:bondValue(h.id,other.id)})).sort((a,b)=>b.value-a.value);
  return `<div class="action-list">${rows.map(({other,value})=>{const unlocked=BOND_MILESTONES.filter(m=>value>=m.level),next=BOND_MILESTONES.find(m=>value<m.level);return `<div class="quest-contract"><span><strong>🤝 ${escapeHTML(other.name)}</strong><small>Bond ${value}/100${unlocked.length?` · ${escapeHTML(unlocked[unlocked.length-1].name)}`:""}</small><small>${next?`Next: ${next.name} at ${next.level}`:"Maximum bond · all synergies active"}</small></span><b>${value}</b></div>`;}).join("")}</div>`;
}
function nemesisMatches(h,run){return !!(h?.nemesis&&run?.enemy&&h.nemesis.combatId===run.combatId&&h.nemesis.enemyName===run.enemy.name);}
function maybeCreateNemesis(run,h,offline=false){
  const enemy=run.enemy;if(!enemy)return;const should=enemy.boss||random()<.35;if(!should)return;
  if(h.nemesis&&h.nemesis.combatId===run.combatId&&h.nemesis.enemyName===enemy.name)h.nemesis.defeats=(h.nemesis.defeats||1)+1;
  else h.nemesis={combatId:run.combatId,enemyName:enemy.name,defeats:1,since:Date.now()};
  if(!offline){postHeroChat(h,`${enemy.name}. Remember that name. I do.`,"danger",{mapBubble:false});notify("Nemesis made",`${enemy.name} has become ${h.name}'s Nemesis.`,"💀");}
}
function resolveNemeses(run,enemy,party,offline=false){
  for(const h of party){if(!h.nemesis||h.nemesis.combatId!==run.combatId||h.nemesis.enemyName!==enemy.name)continue;
    const reward=100+h.level*10;heroEarnGold(h,reward,"nemesis");h.nemesisHistory=(h.nemesisHistory||0)+1;const name=h.nemesis.enemyName;h.nemesis=null;
    if(!offline){postHeroChat(h,`${name} is down. That one was personal.`,"victory",{replyType:"clear",replyChance:.65,mapBubble:false});notify("Nemesis defeated",`${h.name} settled the score with ${name} and earned ${fmt(reward)} personal Gold.`,"⚔️");}
  }
}
...
const ITEMS = {
  ...
  verdantBlade:{name:"Greenwarden’s Edge",type:"weapon",className:"Warrior",icon:"🌿",tier:"Rare Expedition Drop",attack:8,element:"+2 Nature Damage · heal 6% of damage dealt",effects:{lifesteal:.06},requiredLevel:10,value:950,salvage:10,special:true},
  briarRobes:{name:"Raiment of the Briar Saint",type:"armor",className:"Wizard",icon:"🍃",tier:"Rare Expedition Drop",defense:11,element:"Root ward · 8% less damage from enemy specials",effects:{specialDamageReduction:.08},requiredLevel:10,value:980,salvage:10,special:true},
  ...
  darkWand:{name:"Nightwhisper",type:"weapon",className:"Wizard",icon:"🌑",tier:"Dungeon",attack:11,element:"+2 Shadow Damage · 10% chance: Night Surge for 40% of the hit",effects:{voidSurgeChance:.10,voidSurgeMultiplier:.40},requiredLevel:24,value:1850,salvage:16,special:true},
  ...
  coralArmor:{name:"Reefking’s Carapace",type:"armor",className:"Warrior",icon:"🪸",tier:"Dungeon",defense:29,element:"Wave ward · 10% less damage from bosses",effects:{bossDamageReduction:.10},requiredLevel:58,value:6100,salvage:36,special:true},
  stormStaff:{name:"Skyroot",type:"weapon",className:"Druid",icon:"⛈️",tier:"Rare Expedition Drop",attack:25,element:"+6 Lightning Damage · 12% chance: Chain Lightning for 40% of the hit",effects:{chainLightningChance:.12,chainLightningMultiplier:.40},requiredLevel:70,value:7600,salvage:42,special:true},
  ...
  basiliskPlate:{name:"Stonegaze Carapace",type:"armor",className:"Warrior",icon:"🐲",tier:"Raid",defense:22,element:"Poison ward · 50% less venom damage",effects:{poisonResist:.50},requiredLevel:30,value:11500,salvage:75,special:true,raid:true},
  ...
  titanWard:{name:"Mantle of the Mountain Storm",type:"armor",className:"Druid",icon:"🗿",tier:"Raid",defense:38,element:"Party storm ward · party takes 6% less damage while wearer stands",effects:{partyDamageReduction:.06},requiredLevel:60,value:27000,salvage:145,special:true,raid:true},
  ...
  nightweave:{name:"Shroud Beyond Dawn",type:"armor",className:"Assassin",icon:"🕸️",tier:"Raid",defense:52,element:"Eclipse evasion · 8% chance to evade enemy attacks",effects:{evasion:.08},requiredLevel:90,value:72000,salvage:340,special:true,raid:true},
};
...
const HERO_RECORD_DEFAULTS={kills:0,expeditions:0,dungeons:0,raids:0,raidBosses:0,dungeonBosses:0,goldEarned:0,taxPaid:0,guildDonations:0,beers:0,defeats:0,itemsFound:0,foodGathered:0,foodCooked:0,metalMined:0,woodGathered:0,kitsForged:0,workActions:0,secondsActive:0,damageDealt:0,damageTaken:0,healingDone:0,combatActions:0};
...
const ACHIEVEMENTS = [
  ...
  {id:"special",icon:"❄️",name:"Something Special",description:"Find unique Dungeon equipment.",test:s=>s.inventory.some(i=>ITEMS[i.key]?.special)||s.heroes.some(h=>(h.inventory||[]).some(i=>ITEMS[i.key]?.special))},
...
function questBoardHTML(){
  ...
  return `<div class="drawer-section quest-board"><div class="profile-section-title"><div><small>Three postings each day</small><h3>📜 Guild Contracts</h3></div><small>${completed}/3 complete today</small></div><p class="section-copy">The Guild Hall posts three contracts each day. Supply orders are intentionally large town-scale requests; completed slots do not refill until tomorrow.</p><div class="action-list">${state.questBoard.map(q=>{const progress=questProgress(q),done=!q.claimed&&progress>=q.amount,objective=q.type==="kill"?`Defeat ${q.amount} monsters`:`Turn in ${fmt(q.amount)} ${q.resource}`,shown=Math.min(progress,q.amount);return `<button class="quest-contract ${done?"ready":""} ${q.claimed?"claimed":""}" data-action="claim-quest" data-quest="${q.id}" ${done?"":"disabled"}><span><strong>${q.icon} ${escapeHTML(q.title)}</strong><small>${escapeHTML(objective)} · ${fmt(shown)} / ${fmt(q.amount)}</small><small>${escapeHTML(questRewardText(q))}</small></span><b>${q.claimed?"Completed":done?"Claim":"In progress"}</b></button>`;}).join("")}</div></div>`;
}
function claimQuest(id){ensureDailyQuestBoard();const q=state.questBoard.find(q=>q.id===id);if(!q||q.claimed)return;if(questProgress(q)<q.amount)return toast("📜","Quest is not complete yet");if(q.type==="resource"&&!spendQuestResource(q.resource,q.amount))return toast("📦","Those supplies are no longer available");state.resources.gold+=q.reward.gold;state.stats.goldEarned+=q.reward.gold;state.resources.essence+=q.reward.essence||0;state.resources.keys+=q.reward.keys||0;state.stats.questsCompleted=(state.stats.questsCompleted||0)+1;q.claimed=true;notify("Guild contract complete",`${q.title}: ${questRewardText(q)}. ${state.questBoard.filter(x=>x.claimed).length}/3 contracts complete today.`,"📜");markDirty();renderAll();openView("market");}
...
function freshState(){
  return {
    ...
    heroes:HEROES.map((h,i)=>({ ...h, level:1,xp:0,sanity:100,hp:Math.round(50*(CLASS_COMBAT[h.className]?.hp||1)),assignment:"idle", recoveryUntil:0,workProgress:0,workTiers:{farm:"starter",cook:"starter",mine:"starter",forest:"starter"},records:{...HERO_RECORD_DEFAULTS},
      skills:{farming:{level:1,xp:0},cooking:{level:1,xp:0},mining:{level:1,xp:0},woodcutting:{level:1,xp:0},smithing:{level:1,xp:0},plundering:{level:1,xp:0}},gold:100,inventory:[],stance:"balanced",nemesis:null,nemesisHistory:0,
...
function migrate(raw){
  ...
  merged.heroes=base.heroes.map(b=>{const h=raw.heroes.find(x=>x.id===b.id)||{},skills=Object.fromEntries(Object.entries(b.skills).map(([key,value])=>[key,{...value,...(h.skills?.[key]||{})}])),level=clamp(Math.floor(Number(h.level)||1),1,100),legacyProgress=raw.combatXpCurve===1?null:clamp((Number(h.xp)||0)/Math.max(1,xpForLevel(level)),0,.999999),xp=level>=100?0:legacyProgress===null?clamp(Number(h.xp)||0,0,combatXPForLevel(level)-1):Math.floor(combatXPForLevel(level)*legacyProgress);return {...b,...h,level,xp,portrait:b.portrait,workProgress:Math.max(0,Number(h.workProgress)||0),workTiers:{...b.workTiers,...(h.workTiers||{})},records:{...HERO_RECORD_DEFAULTS,...(h.records||{})},skills,inventory:Array.isArray(h.inventory)?h.inventory.filter(i=>i&&ITEMS[i.key]):[],equipment:{...b.equipment,...(h.equipment||{})},stance:COMBAT_STANCES[h.stance]?h.stance:"balanced",nemesis:h.nemesis&&h.nemesis.enemyName?{...h.nemesis}:null,nemesisHistory:Math.max(0,Number(h.nemesisHistory)||0)}});
  merged.combatXpCurve=1;
  merged.inventory=Array.isArray(raw.inventory)?raw.inventory.filter(i=>i&&ITEMS[i.key]):[];merged.bonds={...(raw.bonds||{})};merged.guild={...base.guild,...(raw.guild||{})};
...
function heroAttack(h){const identity=CLASS_COMBAT[h.className]||{},stance=COMBAT_STANCES[h.stance]||COMBAT_STANCES.balanced;return Math.max(1,Math.round((4+h.level*2+(activeEquipment(h,"weapon")?.attack||0)+equipmentEffect(h,"attack"))*(identity.attack||1)*stance.attack));}
function heroDefense(h){const identity=CLASS_COMBAT[h.className]||{},stance=COMBAT_STANCES[h.stance]||COMBAT_STANCES.balanced;return Math.max(1,Math.round((2+h.level+(activeEquipment(h,"armor")?.defense||0)+equipmentEffect(h,"defense"))*(identity.defense||1)*stance.defense));}
...
function heroMaxHit(h){const identity=CLASS_COMBAT[h.className]||{},stance=COMBAT_STANCES[h.stance]||COMBAT_STANCES.balanced;return Math.max(1,Math.round((2+Math.floor(h.level/4)+(activeEquipment(h,"weapon")?.attack||0)+equipmentEffect(h,"attack"))*(identity.maxHit||1)*stance.maxHit));}
...
function heroResolve(h){const taxPenalty=Math.max(0,Math.floor(((state?.taxRate||0)-20)/5))*2;return Math.max(50,Math.round((CLASS_COMBAT[h.className]?.resolve||100)+(h.level-1)/5+equipmentEffect(h,"resolve")-taxPenalty));}
...
function postCombatDanger(run,hero){rememberPartyEvent("lowHP",hero,{hp:Math.max(1,Math.round(hero.hp||1))});postHeroChat(hero,chatPick(HERO_CHAT_VOICES[hero.id].danger),"danger",{cooldownKey:`danger-${hero.id}`,cooldownSeconds:150,replyType:"danger",replyChance:.7,replyPool:run.heroIds});}
...
function rememberPartyEvent(type,hero=null,data={}){if(quietSimulation||!state.chatMeta)return;state.chatMeta.memories=Array.isArray(state.chatMeta.memories)?state.chatMeta.memories:[];state.chatMeta.memories.push({id:uid(),time:Date.now(),type,heroId:hero?.id||null,data});state.chatMeta.memories=state.chatMeta.memories.slice(-20);}
function recentPartyMemory(){const now=Date.now(),memories=(state.chatMeta?.memories||[]).filter(m=>now-m.time<30*60*1000);return memories.length?memories[Math.floor(Math.random()*memories.length)]:null;}
function postMemoryBanter(){const m=recentPartyMemory();if(!m)return false;const h=heroById(m.heroId),orin=heroById("summoner"),vex=heroById("assassin"),mira=heroById("druid");if(m.type==="plunderFail"&&h){postHeroChat(orin,`So... what did ${h.name} bring back?`,"ambient",{mapBubble:false});postHeroChat(h,"Experience.","reply",{mapBubble:false});postHeroChat(vex,"They got caught.","reply");return true;}if(m.type==="unique"&&h){const item=m.data.item||"that unique";if(h.id==="summoner"){postHeroChat(orin,`Still thinking about ${item}. That roll was incredible.`,"ambient",{mapBubble:false});postHeroChat(vex,"Yes. We were all there when probability made a mistake.","reply");return true;}postHeroChat(vex,`${h.name} is still pretending ${item} was skill and not a loot roll.`,"ambient",{mapBubble:false});postHeroChat(h,"Jealousy is an ugly status effect.","reply");return true;}if(m.type==="lowHP"&&h){postHeroChat(mira,`${h.name}, you were at ${m.data.hp||1} health.`,"ambient",{mapBubble:false});postHeroChat(h,"But I was not at zero.","reply",{mapBubble:false});postHeroChat(mira,"That is not the lesson.","reply");return true;}if(m.type==="tax"){postHeroChat(vex,m.data.up?"Oh, excellent. The Guildmaster discovered percentages.":"The tax rate went down. Nobody move. We may frighten it.","ambient",{mapBubble:false});postHeroChat(orin,m.data.up?"I suddenly understand why adventurers become bandits.":"THE ECONOMY IS HEALING.","reply");return true;}return false;}
function postRareFindChat(hero,item){if(!hero)return;rememberPartyEvent("unique",hero,{item:item.name});postHeroChat(hero,HERO_CHAT_VOICES[hero.id].rare(item.name),"loot",{replyType:"rare",replyChance:.58});if(hero.id==="summoner"){const roaster=chatPick(state.heroes.filter(x=>x.id!==hero.id&&x.assignment!=="inn"));const lines=[`${hero.name} actually got the unique. Nobody tell him this validates the summoning thing.`,`Nice roll, ${hero.name}. The ghosts still do not count as extra party members.`,`${hero.name} did something cool. I have documented the anomaly.`];if(roaster)postHeroChat(roaster,chatPick(lines),"reply");}}
...
function maybeAmbientPartyChat(seconds){
  if(quietSimulation||!state.chatMeta)return;state.chatMeta.ambientProgress=(Number(state.chatMeta.ambientProgress)||0)+seconds;if(state.chatMeta.ambientProgress<(Number(state.chatMeta.nextAmbientAt)||55))return;state.chatMeta.ambientProgress=0;state.chatMeta.nextAmbientAt=38+Math.floor(Math.random()*33);if(Math.random()<.18&&postMemoryBanter())return;if(Math.random()<.015){const orin=heroById("summoner"),vex=heroById("assassin");if(orin&&vex){postHeroChat(orin,"You guys would tell me if the summoning thing was getting annoying, right?","ambient",{mapBubble:false});postHeroChat(vex,"It was annoying on day one.","reply",{mapBubble:false});postHeroChat(orin,"Right.","reply",{mapBubble:false});postHeroChat(vex,"Would not be the same without it, though.","reply");return;}}if(Math.random()<.48&&postPairBanter())return;let candidates=state.heroes.filter(hero=>hero.assignment!=="inn");if(candidates.length>1)candidates=candidates.filter(hero=>hero.id!==state.chatMeta.lastSpeakerId);const hero=chatPick(candidates.length?candidates:state.heroes),text=contextualAmbientChat(hero);if(text)postHeroChat(hero,text,"ambient",{cooldownKey:`ambient-${hero.id}-${hero.assignment}`,cooldownSeconds:38});
}
...
function heroEarnGold(h,amount,source="loot"){amount=Math.max(0,Math.floor(amount));const tax=Math.floor(amount*(state.taxRate||0)/100),net=amount-tax;h.gold=(h.gold||0)+net;h.records.goldEarned=(h.records.goldEarned||0)+amount;h.records.taxPaid=(h.records.taxPaid||0)+tax;state.resources.gold+=tax;state.guild.lifetimeTax=(state.guild.lifetimeTax||0)+tax;state.stats.goldEarned+=tax;return {gross:amount,tax,net,source};}
function heroSpendGold(h,amount){amount=Math.max(0,amount);const paid=Math.min(h.gold||0,amount);h.gold=Math.max(0,(h.gold||0)-paid);state.resources.gold+=paid;state.guild.serviceRevenue=(state.guild.serviceRevenue||0)+paid;return paid;}
...
const GUILD_RANKS=[
  {level:1,name:"Newly Chartered",rep:0},{level:2,name:"Local Company",rep:150},{level:3,name:"Bronze Company",rep:450},{level:4,name:"Silver Company",rep:1000},{level:5,name:"Gold Company",rep:2200},{level:6,name:"Renowned Company",rep:4500},{level:7,name:"Heroic Company",rep:9000},{level:8,name:"Legendary Company",rep:18000}
];
function guildRank(){let rank=GUILD_RANKS[0];for(const candidate of GUILD_RANKS)if((state.guild?.reputation||0)>=candidate.rep)rank=candidate;return rank;}
function nextGuildRank(){const rank=guildRank();return GUILD_RANKS.find(candidate=>candidate.level===rank.level+1)||null;}
function heroInventoryCapacity(){return 10+Math.min(4,Math.max(0,guildRank().level-1))+Math.min(2,Math.max(0,(state.buildings?.market||1)-1));}
function heroInventoryCount(h){return (h.inventory||[]).length;}
function ensureHeroInventory(h){if(!Array.isArray(h.inventory))h.inventory=[];return h.inventory;}
function guildDonationRep(item){const d=itemData(item),base=Math.max(1,Math.round(Math.sqrt(Math.max(25,d.value||100))*1.8)),tierBonus=d.raid?60:d.special?25:d.tier==="Starter"?0:8;return base+tierBonus+(d.upgrade||0)*15;}
function putItemInHeroInventory(hero,item,{fallbackWarehouse=true}={}){if(!hero)return false;const inventory=ensureHeroInventory(hero);if(inventory.length<heroInventoryCapacity()){inventory.push(item);return "hero";}if(fallbackWarehouse){state.inventory.push(item);return "warehouse";}return false;}
function findOwnedItem(heroId,itemId){const hero=heroById(heroId);if(!hero)return null;const index=ensureHeroInventory(hero).findIndex(i=>i.id===itemId);return index<0?null:{hero,index,item:hero.inventory[index]};}
function moveHeroItemToWarehouse(heroId,itemId){const found=findOwnedItem(heroId,itemId);if(!found)return;if(occupiedSlots()>=warehouseCapacity())return toast("📦","Warehouse is full");found.hero.inventory.splice(found.index,1);state.inventory.push(found.item);notify("Moved to Warehouse",`${found.hero.name} stored ${itemData(found.item).name} in shared storage.`,"📦");markDirty();renderAll();openHero(heroId);}
function openWarehouseHeroPicker(itemId){const item=state.inventory.find(i=>i.id===itemId);if(!item)return;const d=itemData(item);openDrawer(`Give ${d.name}`,"Warehouse → personal inventory",`<div class="loot-summary"><span>${itemImage(d,"loot-summary-art")}</span><div><strong>${escapeHTML(d.name)}</strong><p>Choose which hero will own and carry this item. A full pack cannot accept another item.</p></div></div><div class="action-list">${state.heroes.map(h=>`<button data-action="warehouse-to-hero" data-item="${itemId}" data-hero="${h.id}" ${heroInventoryCount(h)>=heroInventoryCapacity()?"disabled":""}><span class="inline-hero">${heroImage(h)} ${escapeHTML(h.name)}</span><small>Pack ${heroInventoryCount(h)} / ${heroInventoryCapacity()}</small></button>`).join("")}</div><div class="drawer-footer"><button class="soft-button" data-action="close-drawer">Cancel</button></div>`);}
function moveWarehouseItemToHero(itemId,heroId){const idx=state.inventory.findIndex(i=>i.id===itemId),hero=heroById(heroId);if(idx<0||!hero)return;if(heroInventoryCount(hero)>=heroInventoryCapacity())return toast("🎒",`${hero.name}'s pack is full`);const [item]=state.inventory.splice(idx,1);ensureHeroInventory(hero).push(item);notify("Item transferred",`${itemData(item).name} now belongs to ${hero.name}.`,"🎒");markDirty();renderAll();openHero(heroId);}
function donateGuildItem(source,itemId,heroId=null){let item,owner=null;if(source==="warehouse"){const idx=state.inventory.findIndex(i=>i.id===itemId);if(idx<0)return;[item]=state.inventory.splice(idx,1);}else{const found=findOwnedItem(heroId,itemId);if(!found)return;owner=found.hero;[item]=owner.inventory.splice(found.index,1);}const d=itemData(item),rep=guildDonationRep(item);state.guild.reputation=(state.guild.reputation||0)+rep;state.guild.itemsDonated=(state.guild.itemsDonated||0)+1;if(owner)owner.records.guildDonations=(owner.records.guildDonations||0)+1;notify("Guild donation",`${owner?`${owner.name} donated `:"Donated "}${d.name} for ${rep} Guild Reputation.`,"🏰");const newRank=guildRank();postHeroChat(owner||chatPick(state.heroes),`${d.name} is going into the Guild collection. That's ${rep} reputation toward ${newRank.name}.`,"milestone",{cooldownKey:"guild-donation",cooldownSeconds:15});markDirty();renderAll();if(source==="hero")openHero(heroId);else openView("market");}
function equipOwnedItem(heroId,itemId){const found=findOwnedItem(heroId,itemId);if(!found)return;const {hero,item,index}=found;if(hero.assignment==="combat")return toast("⚔️","Recall this hero before changing equipment");const d=itemData(item),slot=d.type;if(!["weapon","armor","pet","trinket"].includes(slot))return toast("⚠️","That item cannot be equipped");if(d.className&&hero.className!==d.className)return toast("⚠️",`${d.name} is for the ${d.className}`);if(hero.level<(d.requiredLevel||1))return toast("🔒",`${hero.name} needs Combat Level ${d.requiredLevel}`);const previous=hero.equipment[slot];hero.equipment[slot]={...item};hero.inventory.splice(index,1);if(previous)putItemInHeroInventory(hero,{...previous,id:uid()});hero.hp=Math.min(hero.hp??heroMaxHP(hero),heroMaxHP(hero));notify("Equipment changed",`${hero.name} equipped ${d.name}.`,d.icon);markDirty();renderAll();openHero(heroId);}
function heroInventoryHTML(h){const inventory=ensureHeroInventory(h),capacity=heroInventoryCapacity();if(!inventory.length)return `<div class="hero-pack-empty"><span>🎒</span><div><strong>Pack empty</strong><small>Personal loot found by ${escapeHTML(h.name)} will appear here.</small></div></div>`;return `<div class="hero-pack-grid">${inventory.map(item=>{const d=itemData(item),equippable=["weapon","armor","pet","trinket"].includes(d.type),rep=guildDonationRep(item);return `<article class="hero-pack-item ${d.special?"special":""}"><span class="hero-pack-art">${itemImage(d,"item-art")}</span><div><strong>${escapeHTML(d.name)}</strong><small>${escapeHTML(d.tier||"")} ${escapeHTML(d.type||"item")}${d.upgrade?` · +${d.upgrade}`:""}</small><p>${escapeHTML(d.element||d.effectText||"")}</p></div><div class="hero-pack-actions">${equippable?`<button data-action="equip-owned-item" data-hero="${h.id}" data-item="${item.id}">Equip</button>`:""}<button data-action="hero-to-warehouse" data-hero="${h.id}" data-item="${item.id}">Warehouse</button><button data-action="donate-guild-item" data-source="hero" data-hero="${h.id}" data-item="${item.id}">Donate +${rep} Rep</button></div></article>`;}).join("")}</div><p class="pack-capacity-note">🎒 ${inventory.length} / ${capacity} personal slots used. Guild Reputation and Guild Hall upgrades expand every hero's pack up to 16 slots.</p>`;}
function guildHeroOverviewHTML(h){return `<button class="guild-hero-card" data-action="open-hero" data-hero="${h.id}" style="--hero-color:${h.color}"><span class="guild-hero-portrait">${heroImage(h)}</span><span><strong>${escapeHTML(h.name)}</strong><small>${escapeHTML(statusFor(h))}</small></span><span class="guild-hero-economy"><b>🪙 ${fmt(h.gold||0)}</b><small>🎒 ${heroInventoryCount(h)}/${heroInventoryCapacity()}</small></span></button>`;}
function guildTreasuryHTML(){return `<section class="guild-panel treasury-panel"><div class="guild-panel-head"><div><span class="eyebrow">The Guild's cut</span><h3>💰 Treasury & Taxes</h3></div><strong>${state.taxRate}% tax</strong></div><div class="guild-stat-grid"><div><small>Town treasury</small><strong>🪙 ${fmt(state.resources.gold)}</strong></div><div><small>Lifetime tax</small><strong>${fmt(state.guild.lifetimeTax||0)}</strong></div><div><small>Hero service spending</small><strong>${fmt(state.guild.serviceRevenue||0)}</strong></div><div><small>Stipends donated</small><strong>${fmt(state.guild.heroDonations||0)}</strong></div></div><p>Heroes keep their earnings after tax and pay the Guild for meals, Tavern recovery, and Inn care. Taxes above 20% cost the roster 2 Resolve per extra 5%—more revenue means faster Sanity drain in combat. You can fund a struggling hero from the treasury.</p><div class="guild-tax-controls"><button data-action="tax-down">−5%</button><strong>${state.taxRate}%</strong><button data-action="tax-up">+5%</button></div><div class="guild-stipend-grid">${state.heroes.map(h=>`<button data-action="donate-hero" data-hero="${h.id}"><span>${heroImage(h,"tiny-hero-image")}</span><strong>${escapeHTML(h.name)}</strong><small>🪙 ${fmt(h.gold||0)} · Give 100</small></button>`).join("")}</div></section>`;}
function renderGuildHall(){const host=$("#guildHallContent");if(!host)return;ensureDailyQuestBoard();const rank=guildRank(),next=nextGuildRank(),progress=next?clamp(((state.guild.reputation||0)-rank.rep)/(next.rep-rank.rep)*100,0,100):100,cost=buildingUpgradeCost("market");host.innerHTML=`<section class="guild-hero-overview"><div class="guild-panel-head"><div><span class="eyebrow">Six adventurers, one charter</span><h3>Guild Roster</h3></div><strong>${rank.name}</strong></div><div class="guild-roster-grid">${state.heroes.map(guildHeroOverviewHTML).join("")}</div></section><section class="guild-panel reputation-panel"><div class="guild-panel-head"><div><span class="eyebrow">Guild Reputation</span><h3>🏰 ${rank.name}</h3></div><strong>${fmt(state.guild.reputation||0)} Rep</strong></div><div class="guild-rep-meter"><span style="--value:${progress}%"></span></div><p>${next?`${fmt(next.rep-(state.guild.reputation||0))} more Reputation to reach ${next.name}. Donate equipment to build the Guild's collection and reputation. Guild Hall upgrades also expand pack capacity.`:`Your Guild has reached the current maximum rank.`}</p><div class="guild-stat-grid"><div><small>Items donated</small><strong>${fmt(state.guild.itemsDonated||0)}</strong></div><div><small>Hero pack capacity</small><strong>${heroInventoryCapacity()} each</strong></div><div><small>Guild Hall</small><strong>Level ${state.buildings.market}</strong></div><div><small>Next Hall upgrade</small><strong>🪙 ${fmt(cost.gold)} · 🌲 ${fmt(cost.wood)} · ⛏️ ${fmt(cost.metal)}</strong></div></div><button class="primary-button guild-upgrade" data-action="upgrade-building" data-building="market">Upgrade Guild Hall</button></section>${guildTreasuryHTML()}<section class="guild-panel contracts-panel">${questBoardHTML()}</section><section class="guild-panel guild-records"><div class="hero-league-heading"><div><span class="eyebrow">The record wall</span><h3>🏆 Heroes' League</h3><p>Compare the six adventurers by the record that matters to you.</p></div><div class="segmented compact" role="tablist" aria-label="Hero leaderboard category"><button class="active" data-action="hero-league-metric" data-metric="overview">Overview</button><button data-action="hero-league-metric" data-metric="combat">Combat</button><button data-action="hero-league-metric" data-metric="bosses">Bosses</button><button data-action="hero-league-metric" data-metric="work">Work</button><button data-action="hero-league-metric" data-metric="legacy">Legacy</button></div></div><div id="heroLeague" class="hero-league"></div></section>`;renderHeroLeague();}
function awardInventoryItem(key,hero=null,title="Special find!"){
  const d=ITEMS[key];if(!d)return null;const item={id:uid(),key,durability:100,upgrade:0,acquiredAt:Date.now()};let destination="warehouse";if(hero){destination=putItemInHeroInventory(hero,item)||"warehouse";hero.records.itemsFound=(hero.records.itemsFound||0)+1;}else state.inventory.push(item);state.stats.itemsFound++;const overflow=destination==="warehouse"&&occupiedSlots()>warehouseCapacity();const where=destination==="hero"?`${hero.name}'s pack`:"the Warehouse";notify(overflow?"Find held in overflow":title,`${hero?`${hero.name} found `:""}${d.name}${overflow?". Make room in the Warehouse.":` → ${where}.`}`,overflow?"📦":d.icon);playSound("rare");postRareFindChat(hero,d);return item;
}
...
const IDLE_HANGOUTS=[{id:"square",name:"Town Square",pos:[48,54]},{id:"guild",name:"Guild Hall",pos:[42,45]},{id:"tavern",name:"outside the Tavern",pos:[37,72]},{id:"dock",name:"the docks",pos:[84,88]},{id:"forge",name:"watching the Blacksmith",pos:[64,77]},{id:"kitchen",name:"near the Kitchen",pos:[32,63]}];
const HERO_SPENDING={warrior:[[8,"bought a round for the party"],[5,"bought an alarming amount of bread"]],wizard:[[9,"bought a book with an unnecessarily long title"],[6,"bought ink and called it an investment"]],archer:[[5,"replaced some fletching supplies"],[4,"bought trail snacks and denied they were snacks"]],druid:[[6,"bought a new plant pot"],[5,"bought herbs for everyone except herself"]],assassin:[[3,"spent Gold and refuses to elaborate"],[4,"bought a whetstone, apparently the old one was 'embarrassing'"]],summoner:[[12,"bought a suspiciously ornate hat"],[7,"paid for something described only as 'ghost snacks'"]]};
function processPartyLife(seconds){if(quietSimulation||!state.chatMeta)return;state.chatMeta.lifeProgress=(Number(state.chatMeta.lifeProgress)||0)+seconds;if(state.chatMeta.lifeProgress<90)return;state.chatMeta.lifeProgress=0;const idle=state.heroes.filter(h=>h.assignment==="idle");if(idle.length){const h=chatPick(idle),spot=chatPick(IDLE_HANGOUTS);state.chatMeta.idleSpots[h.id]=spot.id;if(random()<.32){const options=HERO_SPENDING[h.id]||[],choice=chatPick(options);if(choice&&h.gold>=choice[0]){h.gold-=choice[0];state.resources.gold+=choice[0];state.guild.serviceRevenue=(state.guild.serviceRevenue||0)+choice[0];postHeroChat(h,`${choice[1]}. There goes ${choice[0]} Gold.`,"ambient",{mapBubble:false});}}}}
...
function enterCurrentRoom(run,cfg,offline=false){
  ...
  if(room.type==="combat"){
    run.enemy=createEnemy(cfg,room);run.roomState={type:"combat"};run.enemyTimer=run.enemy.speed*.7;run.heroTimers={};if(!offline){for(const h of run.heroIds.map(heroById).filter(Boolean))if(nemesisMatches(h,run))postHeroChat(h,`${run.enemy.name}. There you are.`, "danger",{mapBubble:false,cooldownKey:`nemesis-${h.id}-${run.enemy.name}`,cooldownSeconds:300});}
...
function performHeroAttack(run,h,offline=false){
  const enemy=run.enemy;if(!enemy||(h.hp||0)<=0)return;const stats=combatRunHeroStats(run,h.id);stats.attacks++;h.records.combatActions=(h.records.combatActions||0)+1;const maxHit=heroMaxHit(h),bond=partyBondBonus(run,h),hitChance=clamp(.25+(heroAttack(h)/(heroAttack(h)+enemy.defense))*.7,.25,.98);let damage=random()<hitChance?Math.floor(random()*(maxHit+1)):0,crit=damage>0&&random()<clamp(heroCritChance(h)+bond.crit,0,.75),special="";if(damage)damage=Math.max(1,Math.round(damage*bond.damage));if(damage)stats.hits++;if(crit)stats.crits++;
  ...
  if(damage&&equipmentEffect(h,"lifesteal")>0){const healed=Math.min(heroMaxHP(h)-(h.hp||0),Math.max(1,Math.round(damage*equipmentEffect(h,"lifesteal"))));if(healed>0){h.hp+=healed;combatRunHeroStats(run,h.id).healing+=healed;pushCombatEvent(run,`${h.name}'s weapon restores ${healed} HP.`,"heal",h.id,healed,h.id,offline);}}
...
function performEnemyAttack(run,offline=false){
  ...
  let totalDamage=0;for(const h of targets){const evasion=equipmentEffect(h,"evasion");if(evasion&&random()<evasion){pushCombatEvent(run,`${h.name} evades ${enemy.name}'s attack.`,"miss",h.id,0,"enemy",offline);continue;}const hitChance=clamp(.22+(enemy.attack/(enemy.attack+heroDefense(h)*1.8))*.68,.18,.95),base=random()<hitChance?Math.floor(random()*(enemy.maxHit+1)):0,specialMult=trigger&&special.id==="crusher"?1.75:1,raw=targets.length>1?Math.ceil(base*.65):Math.ceil(base*specialMult),multiplier=combatStyleMultiplier(enemy.combatStyle,heroCombatStyle(h)),stance=COMBAT_STANCES[h.stance]||COMBAT_STANCES.balanced,bond=partyBondBonus(run,h),ward=run.heroIds.map(heroById).filter(Boolean).reduce((m,ally)=>Math.max(m,equipmentEffect(ally,"partyDamageReduction")),0),specialReduction=trigger?equipmentEffect(h,"specialDamageReduction"):0,bossReduction=enemy.boss?equipmentEffect(h,"bossDamageReduction"):0,damageMult=stance.damageTaken*bond.taken*(1-ward)*(1-specialReduction)*(1-bossReduction),damage=raw>0?Math.max(1,Math.round(raw*multiplier*damageMult)):0,maxHP=heroMaxHP(h),before=h.hp??maxHP;h.hp=Math.max(0,before-damage);const actual=before-h.hp;totalDamage+=actual;run.damageTaken+=actual;combatRunHeroStats(run,h.id).taken+=actual;h.records.damageTaken=(h.records.damageTaken||0)+actual;const edge=multiplier>1?" · advantage":multiplier<1?" · resisted":"";pushCombatEvent(run,actual?`${enemy.name} hits ${h.name} for ${actual}${edge}.`:`${enemy.name} misses ${h.name}.`,actual?"enemy":"miss",h.id,actual,"enemy",offline);if(trigger&&special.id==="venomous"&&actual&&h.hp>0){const venom=Math.max(1,Math.round(enemy.maxHit*.18*(1-equipmentEffect(h,"poisonResist"))));h.hp=Math.max(0,h.hp-venom);run.damageTaken+=venom;combatRunHeroStats(run,h.id).taken+=venom;h.records.damageTaken=(h.records.damageTaken||0)+venom;pushCombatEvent(run,`${h.name} suffers ${venom} venom damage.`,"poison",h.id,venom,"enemy",offline);}if(!offline&&actual)playSound("enemy");if(h.hp>0&&before/maxHP>.42&&h.hp/maxHP<=.42)postCombatDanger(run,h);if(h.hp<=0)defeatHero(run,h,offline);}
...
function consumeCombatFood(run,h,offline=false){
  if((h.hp||0)>heroMaxHP(h)*.45)return false;if((h.gold||0)<2)return false;if(state.resources.food<=0){postNoFoodChat(h);return false;}const deficit=heroMaxHP(h)-h.hp,bonus=equipmentEffect(h,"foodEfficiency")*4,stance=COMBAT_STANCES[h.stance]||COMBAT_STANCES.balanced,available=RESOURCE_TIERS.food.filter(t=>(state.resourceTiers.food[t.id]||0)>0).map(t=>({...t,actualHeal:Math.round((t.heal+bonus)*stance.food)}));if(!available.length){postNoFoodChat(h);return false;}const tier=available.find(t=>t.actualHeal>=deficit)||available[available.length-1];state.resourceTiers.food[tier.id]--;recalculateTieredTotal("food");heroSpendGold(h,2);const healed=Math.min(deficit,tier.actualHeal);h.hp+=healed;run.foodEaten++;pushCombatEvent(run,`${h.name} eats ${tier.name} and heals ${healed} HP.`,"heal",h.id,healed,h.id,offline);return true;
}
...
function raidIndividualUniqueChance(teamChance,partySize){const n=Math.max(1,partySize);return 1-Math.pow(1-clamp(teamChance,0,.999999),1/n);}
function awardRaidCommonRoll(cfg,hero){const gold=Math.max(1,Math.round((cfg.entryGold||100)*(.10+random()*.08))),pay=heroEarnGold(hero,gold,"raid common roll"),resource=["food","wood","metal"][Math.floor(random()*3)],tiers=RESOURCE_TIERS[resource].filter(t=>t.level<=cfg.minLevel),tier=tiers[Math.floor(random()*tiers.length)]||RESOURCE_TIERS[resource][0],scale=Math.max(1,Math.sqrt(cfg.minLevel||1)),qty=Math.max(2,Math.round((2+random()*4)*scale));addTieredResource(resource,tier.id,qty);return {rare:false,gold,tax:pay.tax,resource,resourceName:tier.name,qty};}
function rollRaidLootForHero(cfg,hero,teamChance,partySize){const chance=raidIndividualUniqueChance(teamChance,partySize);if(cfg.pool?.length&&random()<chance){const key=cfg.pool[Math.floor(random()*cfg.pool.length)],item=awardInventoryItem(key,hero,"Raid unique!");if(item){const d=itemData(item),carried=(hero.inventory||[]).some(i=>i.id===item.id);postHeroChat(hero,carried?`I rolled ${d.name}! That's staying in my pack until the Guildmaster says otherwise.`:`I rolled ${d.name}! My pack was full, so it's waiting for me in the Warehouse.`,"loot",{replyType:"rare",replyChance:.8});}return {rare:true,key,chance};}return {...awardRaidCommonRoll(cfg,hero),chance};}
function raidLootSummaryHTML(results){return results.map(result=>{const h=heroById(result.heroId);if(!h)return "";if(result.rare){const d=ITEMS[result.key];return `${h.name}: ⭐ ${d?.name||"Raid unique"}`;}return `${h.name}: ${fmt(result.gold)} Gold + ${result.qty} ${result.resourceName}`;}).join(" · ");}
function completeCombatCycle(run,cfg,offline=false){
  const party=run.heroIds.map(heroById).filter(Boolean);if(!party.length){stopRun(run.id,false,false,"Party defeated");return;}const gold=Math.floor(cfg.gold[0]+random()*(cfg.gold[1]-cfg.gold[0]+1)),raidScore=cfg.category==="raid"?raidRewardScore(run,cfg):null,share=Math.floor(gold/party.length);for(const h of party)heroEarnGold(h,share,"chest");run.cycles++;run.lastReward=raidScore?`${fmt(gold)} Gold · ${chanceLabel(raidScore.chance)} team unique chance`:`${fmt(gold)} Gold`;
...
function renderAll(){renderResources();renderTown();renderHeroes();renderAssignments();renderCombat();renderWarehouse();renderGuildHall();renderProgress();renderSyncUser();renderPartyChat();}
...
function mapPosition(h,index){
  ...
  const spot=IDLE_HANGOUTS.find(x=>x.id===state.chatMeta?.idleSpots?.[h.id]);if(spot)return [spot.pos[0]+((index%2)?2:-2),spot.pos[1]+(index%3)];
...
function openLoot(combatId){const c=COMBAT[combatId];if(!c)return;const equipmentChance=c.category==="raid"?`Raid loot is performance-based. Damage dealt earns combat points, skill rooms award more points when the party brings higher relevant skills, and clear time weights the final score. The displayed percentage is the team chance for at least one unique; it is converted into one independent loot roll for every participating hero, so a four-hero party gets four personal rolls without multiplying the intended overall unique rate. Every miss still gives that hero a personal Gold + supply bundle.`:c.pool?.length?`Each victory has one ${chanceLabel(c.itemChance)} equipment roll. If it succeeds, every listed item is equally likely.`:"This activity does not drop unique equipment.";openDrawer(`${c.name} Rewards`,"Possible loot and exact rates",`<div class="loot-summary"><span>${c.icon}</span><div><strong>Victory rewards</strong><p>${equipmentChance}</p></div></div><div class="loot-table">${lootRows(c).map(row=>`<div class="loot-row"><span class="loot-icon">${itemImage(row,"loot-art")}</span><div><strong>${escapeHTML(row.name)}</strong><small>${escapeHTML(row.detail)}</small></div><b>${row.chance}</b></div>`).join("")}</div><p class="loot-note">${c.category==="raid"?`Raid target time: ${formatDuration(c.duration)}. High relevant work skills both clear skill rooms faster and award more skill points. Each hero owns any unique they roll in their personal inventory. Boss eggs remain an independent party roll.`:`Drop chances are per completed victory. Independent rolls—such as trinkets, keys, and boss eggs—can occur alongside an equipment drop.`}</p><div class="drawer-footer"><button class="soft-button" data-action="close-drawer">Close</button><button class="primary-button" data-action="open-combat" data-combat="${c.id}">Prepare ${c.short}</button></div>`);}
...
function renderWarehouse(){
  ...
  const resourceCards=resourceList.map(stack=>`<article class="item-card resource-stack"><div class="item-icon">${stack.icon}</div><div><h4>${escapeHTML(stack.name)}</h4><span class="item-meta">${escapeHTML(stack.category)} · Shared Guild resource</span><p>${escapeHTML(stack.detail)}</p></div><div class="resource-quantity"><small>Stored</small><strong>×${fmt(stack.quantity)}</strong></div></article>`);
...
const HERO_LEAGUE_CATEGORIES={
  overview:{label:"Overview",metrics:[
    {key:"totalLevel",icon:"🏆",label:"Total Level",detail:"Combat plus all six work skills"},
  ...
  work:{label:"Work",metrics:[
    {key:"workLevel",icon:"📈",label:"Total Work Level",detail:"All six independent skills"},
...
function openBuilding(id){
  const b=BUILDINGS[id],level=state.buildings[id],cost=buildingUpgradeCost(id),workers=state.heroes.filter(hero=>hero.assignment===id),activity=id==="tavern"?tavernMarketHTML():"",tasks=resourceTierHTML(id)||(id==="smith"?smithWorkTaskHTML():"");
...
function changeTax(delta){const before=state.taxRate;state.taxRate=clamp((state.taxRate||0)+delta,0,50);if(state.taxRate!==before){rememberPartyEvent("tax",null,{up:state.taxRate>before,rate:state.taxRate});const speaker=state.taxRate>before?heroById("assassin"):heroById("summoner");if(speaker)postHeroChat(speaker,state.taxRate>before?"The Guildmaster raised taxes. I am adding pitchforks to my shopping list.":"Taxes went down! I knew relentless optimism would eventually affect fiscal policy.","guild");}markDirty();renderAll();openView("market");}
...
document.addEventListener("click",async event=>{
  ...
  else if(a==="give-item")openWarehouseHeroPicker(b.dataset.item);
  else if(a==="warehouse-to-hero")moveWarehouseItemToHero(b.dataset.item,b.dataset.hero);
  else if(a==="hero-to-warehouse")moveHeroItemToWarehouse(b.dataset.hero,b.dataset.item);
  else if(a==="equip-owned-item")equipOwnedItem(b.dataset.hero,b.dataset.item);
  else if(a==="donate-guild-item" && await confirmAction("Donate this item to the Guild?","The item will be permanently removed and converted into Guild Reputation.","🏰"))donateGuildItem(b.dataset.source,b.dataset.item,b.dataset.hero);
