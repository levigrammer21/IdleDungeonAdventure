...
const CLASS_COMBAT = {
  ...
  Summoner:{speed:2.62,crit:.06,style:"magic",verb:"commands",attack:1.06,defense:.9,hp:.92,maxHit:1.02,resolve:100,identity:"Echo damage"},
};
...
const WORK_ASSIGNMENTS=["farm","mine","forest","smith"];
...
function workTasksFor(assignment){return RESOURCE_ASSIGNMENTS[assignment]?RESOURCE_TIERS[RESOURCE_ASSIGNMENTS[assignment]]:assignment==="smith"?SMITH_WORK_TASKS:[];}
function unlockedWorkTasks(h,assignment){if(RESOURCE_ASSIGNMENTS[assignment])return unlockedResourceTiers(h,assignment);if(assignment!=="smith")return [];const skill=BUILDINGS[assignment].skill,buildingLevel=state.buildings[assignment]||1;return SMITH_WORK_TASKS.filter(task=>(h.skills[skill]?.level||1)>=task.level&&buildingLevel>=task.building);}
...
function workActionTime(h,assignment,tier=null){const skillKey=BUILDINGS[assignment]?.skill,skill=h.skills[skillKey]?.level||1,building=state.buildings[assignment]||1,base=assignment==="smith"?SMITH_WORK_TASKS[0].baseSeconds:TIER_ACTION_SECONDS[tier?.id||"starter"];return base/(1+(skill-1)*.01+(building-1)*.08);}
...
function restoreTavernSanity(h,seconds){
  const potential=Math.min(100-h.sanity,(1.2+state.buildings.tavern*.35)*seconds),restored=Math.min(potential,state.resources.gold/.045);if(restored<=0)return 0;h.sanity=clamp(h.sanity+restored,0,100);state.resources.gold=Math.max(0,state.resources.gold-restored*.045);return restored;
}
...
function advanceCombatRoom(run,cfg,offline=false){run.roomIndex++;run.roomState=null;run.enemy=null;if(run.roomIndex>=COMBAT_LAYOUTS[cfg.id].length)completeCombatCycle(run,cfg,offline);else{rejoinReadyExpeditionHeroes(run,offline);enterCurrentRoom(run,cfg,offline);}}
...
function raidTargetPoints(cfg){
  const layout=COMBAT_LAYOUTS[cfg.id]||[],combat=layout.filter(room=>room.type==="combat").reduce((total,room)=>total+createEnemy(cfg,room).maxHP,0),skill=layout.filter(room=>room.type==="skill").reduce((total,room)=>total+room.baseSeconds*1.5,0);return Math.max(1,Math.round(combat+skill));
}
...
function completeCombatCycle(run,cfg,offline=false){
  ...
  if(!run.autoRepeat){stopRun(run.id,false,false,"Single clear complete");return;}run.cycle++;run.roomIndex=0;run.roomState=null;run.enemy=null;run.cycleElapsed=0;run.raidCombatPoints=0;run.raidSkillPoints=0;
  ...
  if(!canPayRunEntry(cfg)){stopRun(run.id,false,false,"Entry supplies exhausted");notify("Run paused",`${cfg.short} stopped because the next entry needs more ${cfg.keys?"Raid Keys or Essence":"Essence"}.`,"🎒");return;}payRunEntry(cfg);
}
...
function renderActiveRuns(){
  ...
  host.innerHTML=state.combatRuns.map(run=>{
    const cfg=COMBAT[run.combatId],party=run.heroIds.map(heroById).filter(Boolean),resting=(run.restingHeroIds||[]).map(heroById).filter(Boolean),room=COMBAT_LAYOUTS[cfg.id]?.[run.roomIndex],pct=run.waitingForParty?0:roomProgress(run)*100,status=run.waitingForParty?`Waiting for ${resting.map(hero=>hero.name).join(", ")} at the Tavern`:run.roomState?.type==="skill"?`${formatDuration(Math.max(0,run.roomState.remaining))} on obstacle`:run.enemy?`${fmt(run.enemy.hp)} / ${fmt(run.enemy.maxHP)} enemy HP`:"Entering room",roster=[party.length?`${party.length} fighting`:"No one fighting",resting.length?`${resting.length} on break`:""].filter(Boolean).join(" · ");
...
function taskOutputDetail(assignment,task){
  if(assignment==="farm")return `Heals ${task.heal} HP each · ${workTaskTime(task)}s base`;
...
function resourceTierHTML(id){
  const resource=RESOURCE_ASSIGNMENTS[id];if(!resource)return "";const b=BUILDINGS[id],buildingLevel=state.buildings[id]||1;
  const title=resource==="food"?"Food Tasks":resource==="metal"?"Mining Tasks":"Woodcutting Tasks",explanation=resource==="food"?"Every completed action creates one meal item. Higher meals restore more HP when a hero auto-eats in combat; they never convert into generic Food.":`Every completed action creates one distinct ${resource} item. Each stack is its own crafting material and cannot substitute for another tier.`;
...
function smithCraftHTML(){
  ...
  return `<div class="drawer-section"><h3>Manual Equipment Crafting</h3><p class="tier-explainer">Normal equipment follows the same eight exact material tiers as Mining and Woodcutting. These are deliberate one-time crafts, while assigned Smiths continuously make the selected work item above.</p><div class="smith-capability"><div><small>Highest Smithing</small><strong>Level ${capability.level}</strong></div><div><small>Blacksmith</small><strong>Level ${capability.building}</strong></div><div><small>Next equipment tier</small><strong>${nextTier?`${nextTier.name} · Smithing ${nextTier.smithLevel} / Blacksmith ${nextTier.building}`:"All tiers mastered"}</strong></div></div><div class="smith-tier-list">${NORMAL_GEAR_TIER_SPECS.map(tier=>smithTierHTML(tier,capability,currentTier)).join("")}</div></div>`;
}
...
function openCombat(combatId){
  const c=COMBAT[combatId];if(!c)return;const available=state.heroes.filter(h=>!heroReservedForCombat(h.id)&&h.assignment!=="combat"&&h.assignment!=="inn"&&h.sanity>0&&(h.hp||0)>0),max=c.maxParty,entry=[c.keys?`${c.keys} Key${c.keys===1?"":"s"}`:"",c.essence?`${c.essence} Essence`:""].filter(Boolean).join(" + ")||"Free entry",layout=COMBAT_LAYOUTS[c.id];
